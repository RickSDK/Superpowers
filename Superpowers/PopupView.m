...
- (void)commonInit
{
	...
	self.layer.borderColor = [ObjectiveCScripts primaryButtonColor].CGColor;
	self.layer.borderWidth = 1;
	self.backgroundColor = [ObjectiveCScripts themeBGColor];
	...
	if(self.tag==1) { // just background
		...
		self.titleLabel.text = @"Welcome to Superpowers!";
	}
...
- (void)layoutSubviews {
	...
	self.insideView.frame = CGRectMake(3, 3, self.frame.size.width-6, self.frame.size.height-6);
	self.xButton.frame = CGRectMake(self.frame.size.width-29, -2, 30, 30);
	self.titleLabel.frame = CGRectMake(20, 5, self.frame.size.width-40, 25);
	self.textView.frame = CGRectMake(20, 35, self.frame.size.width-40, self.frame.size.height-40);
}
